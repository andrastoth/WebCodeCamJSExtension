...
function ECBlocks(ecCodewordsPerBlock, ecBlocks1, ecBlocks2) {
    ...
    this.__defineGetter__("NumBlocks", function() {
        var total = 0;
        for (var i = 0; i < this.ecBlocks.length; i++) total += this.ecBlocks[i].length;
        return total
    });
    ...
    this.processFinderPatternInfo = function(info) {
        ...
        if (provisionalVersion.AlignmentPatternCenters.length > 0) {
            ...
            while (alignmentPattern === null && i < 16) {
                alignmentPattern = this.findAlignmentInRegion(moduleSize, estAlignmentX, estAlignmentY, i <<= 1);
            }
        }
        var transform = this.createTransform(topLeft, topRight, bottomLeft, alignmentPattern, dimension);
        var bits = this.sampleGrid(this.image, transform, dimension);
        var points;
        if (alignmentPattern == null) points = new Array(bottomLeft, topLeft, topRight);
...
function BitMatrix(width, height) {
    ...
    this.setRegion = function(left, top, width, height) {
        ...
        for (var y = top; y < bottom; y++) {
            ...
            for (var x = left; x < right; x++) this.bits[offset + (x >> 5)] |= 1 << (x & 31)
        }
    }
}
...
DataBlock.getDataBlocks = function(rawCodewords, version, ecLevel) {
    ...
    for (var i = 0; i < shorterBlocksNumDataCodewords; i++)
        for (var j = 0; j < numResultBlocks; j++) result[j].codewords[i] = rawCodewords[rawCodewordsOffset++];
...
function GF256Poly(field, coefficients) {
    ...
    this.evaluateAt = function(a) {
        ...
        if (a == 1) {
            var result = 0;
            for (var i = 0; i < size; i++) result = GF256.addOrSubtract(result, this.coefficients[i]);
            return result
        }
        var result2 = this.coefficients[0];
        for (var i = 1; i < size; i++) result2 = GF256.addOrSubtract(this.field.multiply(a, result2), this.coefficients[i]);
        return result2
    };
    ...
    this.multiply1 = function(other) {
        ...
        for (var i = 0; i < aLength; i++) {
            ...
            for (var j = 0; j < bLength; j++) product[i + j] = GF256.addOrSubtract(product[i + j], this.field.multiply(aCoeff, bCoefficients[j]))
        }
...
qrcode.getMiddleBrightnessPerArea = function(image) {
    ...
    for (var ay = 0; ay < numSqrtArea; ay++)
        for (var ax = 0; ax < numSqrtArea; ax++) middle[ax][ay] = Math.floor((minmax[ax][ay][0] + minmax[ax][ay][1]) / 2);
    return middle
};
qrcode.grayScaleToBitmap = function(grayScale) {
    ...
    for (var ay = 0; ay < sqrtNumArea; ay++)
        for (var ax = 0; ax < sqrtNumArea; ax++)
            for (var dy = 0; dy < areaHeight; dy++)
                for (var dx = 0; dx < areaWidth; dx++) bitmap[areaWidth * ax + dx + (areaHeight * ay + dy) * qrcode.width] = grayScale[areaWidth * ax + dx + (areaHeight * ay + dy) * qrcode.width] < middle[ax][ay] ? true : false;
    return bitmap
};
...
function FinderPatternFinder() {
    ...
    this.selectBestPatterns = function() {
        ...
        if (this.possibleCenters.length > 3) this.possibleCenters.sort(function(a, b) {
                ...
                if (currentState == 4)
                    if (this.foundPatternCross(stateCount)) {
                        ...
                        if (confirmed) {
                            ...
                            do j++; while (j < maxJ && !image[j + i * qrcode.width]);
                            j--
                        }
                        currentState = 0;
                        stateCount[0] = 0;
